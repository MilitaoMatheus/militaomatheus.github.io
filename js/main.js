...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* =========================================================================
     3.6 CARDS DE EXPERIÊNCIA & EDUCAÇÃO (MODAL OVERLAY EM EVIDÊNCIA COM BLUR)
     ========================================================================= */
  const expCards = document.querySelectorAll('.exp-card');
  const expModalBackdrop = document.getElementById('exp-modal-backdrop');
  const expModalWrapper = document.getElementById('exp-modal-wrapper');
  const expModalIcon = document.getElementById('exp-modal-icon');
  const expModalYear = document.getElementById('exp-modal-year');
  const expModalTitle = document.getElementById('exp-modal-title');
  const expModalCompany = document.getElementById('exp-modal-company');
  const expModalDesc = document.getElementById('exp-modal-desc');
  const expModalTags = document.getElementById('exp-modal-tags');
  const expModalCloseBtn = document.getElementById('exp-modal-close-btn');

  const openExpModal = (card) => {
    const year = card.querySelector('.exp-card-year')?.textContent.trim() || '';
    const iconHtml = card.querySelector('.exp-card-icon')?.innerHTML || '';
    const title = card.querySelector('.exp-card-title')?.textContent.trim() || '';
    const company = card.querySelector('.exp-card-company')?.textContent.trim() || '';
    const desc = card.querySelector('.exp-full-desc')?.textContent.trim() || '';
    const tagsHtml = card.querySelector('.exp-tags-list')?.innerHTML || '';

    if (expModalYear) expModalYear.textContent = year;
    if (expModalIcon) expModalIcon.innerHTML = iconHtml;
    if (expModalTitle) expModalTitle.textContent = title;
    if (expModalCompany) expModalCompany.textContent = company;
    if (expModalDesc) expModalDesc.textContent = desc;
    if (expModalTags) expModalTags.innerHTML = tagsHtml;

    expModalBackdrop?.classList.add('open');
    expModalWrapper?.classList.add('open');
    expModalWrapper?.setAttribute('aria-hidden', 'false');
    document.body.classList.add('modal-open');
  };

  const closeExpModal = () => {
    expModalBackdrop?.classList.remove('open');
    expModalWrapper?.classList.remove('open');
    expModalWrapper?.setAttribute('aria-hidden', 'true');
    document.body.classList.remove('modal-open');
  };
  ...
  expModalCloseBtn?.addEventListener('click', closeExpModal);
  expModalBackdrop?.addEventListener('click', closeExpModal);
  expModalWrapper?.addEventListener('click', (e) => {
    if (e.target === expModalWrapper) {
      closeExpModal();
    }
  });

  window.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && expModalWrapper?.classList.contains('open')) {
      closeExpModal();
    }
  });
